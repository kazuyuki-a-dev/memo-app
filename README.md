...
PHP + MySQL(Docker)で構築した、シンプルなメモ管理アプリです。フレームワークを使わず、あえて素のPHPのみで実装しました。会員登録・ログイン機能を備え、ユーザーごとにメモの作成・編集・削除・検索ができます。

## このプロジェクトについて

Laravelなどのフレームワークを使わず、あえて素のPHP(Vanilla PHP)のみで実装しています。フレームワークが裏側で自動化してくれる部分(ルーティング、バリデーション、SQLの発行、セッション管理など)を、すべて自分の手で組み立てることで、PHPそのものの仕組みへの理解を深めることを目的としています。
